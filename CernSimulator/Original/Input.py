...
Dati_sist = '0_Dati di sistema.xlsx'
...
Dati_economici = pd.read_excel('1_Dati.xlsx', sheet_name='Economici', header=None).to_numpy()
Dati_generali = pd.read_excel('1_Dati.xlsx', sheet_name='Generali', header=None).to_numpy()
Dati_tecn = pd.read_excel('1_Dati.xlsx', sheet_name='Tecnici', header=None).to_numpy()
...
# Zona di mercato
Zona = Dati_generali[1, 0]

# PUN: Prezzo unico nazionale
PUN_mod = Dati_generali[6, 0]
PUN = np.zeros(8760)
...
if PUN_mod == 0:
    ...
    PUN_y = Dati_generali[7, 0]                                                #[€/MWh]
    
    indice1PUN = np.where(Dati_sist_Fasce[:, Anno-2019] == 1)
    PUN[indice1PUN] = PUN_y * 100 / 1000 * (1 + Dati_generali[20, 0] / 100)    #[c€/kWh]

    indice2PUN = np.where(Dati_sist_Fasce[:, Anno-2019] == 2)
    PUN[indice2PUN] = PUN_y * 100 / 1000 * (1 + Dati_generali[21, 0] / 100)    #[c€/kWh]

    indice3PUN = np.where(Dati_sist_Fasce[:, Anno-2019] == 3)
    PUN[indice3PUN] = PUN_y * 100 / 1000 * (1 + Dati_generali[22, 0] / 100)    #[c€/kWh]

elif PUN_mod == 2:
    PUN_gen = Dati_generali[8, 0]                                              #[€/MWh]
    PUN_feb = Dati_generali[9, 0]                                              #[€/MWh]
    PUN_mar = Dati_generali[10, 0]                                             #[€/MWh]
    PUN_apr = Dati_generali[11, 0]                                             #[€/MWh]
    PUN_mag = Dati_generali[12, 0]                                             #[€/MWh]
    PUN_giu = Dati_generali[13, 0]                                             #[€/MWh]
    PUN_lug = Dati_generali[14, 0]                                             #[€/MWh]
    PUN_ago = Dati_generali[15, 0]                                             #[€/MWh]
    PUN_set = Dati_generali[16, 0]                                             #[€/MWh]
    PUN_ott = Dati_generali[17, 0]                                             #[€/MWh]
    PUN_nov = Dati_generali[18, 0]                                             #[€/MWh]
    PUN_dic = Dati_generali[19, 0]                                             #[€/MWh]
    ...
    indice1PUN = np.where(Dati_sist_Fasce[:, Anno-2019] == 1)         
    PUN_molt[indice1PUN] = 1 + Dati_generali[20, 0] / 100                      #[%]
 
    indice2PUN = np.where(Dati_sist_Fasce[:, Anno-2019] == 2)
    PUN_molt[indice2PUN] = 1 + Dati_generali[21, 0] / 100                      #[%]

    indice3PUN = np.where(Dati_sist_Fasce[:, Anno-2019] == 3)
    PUN_molt[indice3PUN] = 1 + Dati_generali[22, 0] / 100                      #[%]
...
Input['R'] = Dati_generali[3]                                                  #[%]
Input['Ri'] = Dati_generali[4]                                                 #[%]
Input['BTAU'] = Matrice_BTAU                                                   #[c€/kWh]
Input['Tras_e'] = Matrice_Tras_e                                               #[c€/kWh]
Input['N_clust'] = Numero_di_cluster
Input['Configurazione'] = Dati_generali[0]                                     # 1:comunità energetica, 2:autoconsumo collettivo
Input['Zona'] = Dati_generali[1]
Input['Zona_it'] = Dati_generali[2]
